    ...
    */
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('fields', $value)) {
            $this->_usedProperties['fields'] = true;
            $this->fields = array_map(fn ($v) => new \Symfony\Config\SyliusGrid\GridsConfig\FieldsConfig($v), $value['fields']);
            unset($value['fields']);
        }

        if (array_key_exists('filters', $value)) {
            $this->_usedProperties['filters'] = true;
            $this->filters = array_map(fn ($v) => new \Symfony\Config\SyliusGrid\GridsConfig\FiltersConfig($v), $value['filters']);
            unset($value['filters']);
        }

        if (array_key_exists('actions', $value)) {
            $this->_usedProperties['actions'] = true;
            $this->actions = array_map(fn ($v) => new \Symfony\Config\SyliusGrid\GridsConfig\ActionsConfig($v), $value['actions']);
            unset($value['actions']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['actions'])) {
            $output['actions'] = array_map(fn ($v) => $v->toArray(), $this->actions);
        }
